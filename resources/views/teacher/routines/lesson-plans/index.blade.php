...
@section('title', 'Teacher | Lesson Plans')
@section('content')
                            ...
                            <select name="subject_id" id="subject_id" class="form-select" required>
                                ...
                                @if (isset($subjects) && !empty($subjects))
                                    @foreach ($subjects as $subject)
                                        <option value="{{ $subject->id }}" data-class-id="{{ $subject->class_id }}">{{ $subject->name }} ({{ $subject->code }})</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="dropdown me-2">
                        ...
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0 border-0" id="filter-dropdown">
                            <div class="card mb-0">
                                ...
                                <form action="{{ route('teacher.lesson-plans.index') }}" method="GET" id="filter-form">
                                    <div class="card-body">
                                        <div class="mb-3">
                                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <tr data-lesson-plan-id="{{ $lessonPlan->id }}">
                                    ...
                                    <td>
                                        @if($lessonPlan->lesson_plan_file)
                                            ...
                                            <a href="{{ route('teacher.lesson-plans.download', $lessonPlan->id) }}" 
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (clearAllLink) {
            clearAllLink.addEventListener('click', function() {
                window.location.href = '{{ route("teacher.lesson-plans.index") }}';
            });
        }
...
@push('scripts')
<script src="{{ asset('custom/js/teacher/lesson-plans.js') }}"></script>
@endpush
