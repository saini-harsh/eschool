...
class RoutineController extends Controller
{
    ...
    private function checkRoutineConflicts(Request $request)
    {
        ...
        $existingRoutine = Routine::where('class_id', $request->class_id)
            ->where('section_id', $request->section_id)
        ...
        $teacherConflict = Routine::where('teacher_id', $request->teacher_id)
            ->where('day', $request->day)
            ->where('start_time', $request->start_time)
            ->where('status', 1)
            ->first();
        ...
        if ($request->class_room_id) {
            $roomConflict = Routine::where('class_room_id', $request->class_room_id)
                ->where('day', $request->day)
                ->where('start_time', $request->start_time)
                ->where('status', 1)
                ->first();
