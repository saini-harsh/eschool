...
class LessonPlanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Build query for lesson plans - only show current teacher's lesson plans
        $query = LessonPlan::with(['institution', 'teacher', 'schoolClass', 'subject', 'createdBy'])
            ->where('institution_id', $institutionId)
            ->where('teacher_id', $teacherId); // Only show current teacher's lesson plans
        ...
        return view('teacher.routines.lesson-plans.index', compact('lessonPlans', 'classes', 'subjects', 'currentTeacher'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $lessonPlan = LessonPlan::create([
                ...
                'institution_id' => $institutionId, // Auto-set from logged-in teacher
                'teacher_id' => $teacherId, // Auto-set from logged-in teacher
                'class_id' => $request->class_id,
                'subject_id' => $request->subject_id,
                'lesson_plan_file' => $filePath,
                'status' => $request->status ?? 1,
                'created_by' => $teacherId, // Set as teacher ID instead of admin
            ]);
    ...
    {
        try {
            $lessonPlan = LessonPlan::with(['institution', 'teacher', 'schoolClass', 'subject'])->findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // Ensure the lesson plan belongs to the current teacher's institution
            if ($lessonPlan->institution_id !== $institutionId) {
                return response()->json([
                    'success' => false,
                    'message' => 'Unauthorized access to lesson plan'
                ], 403);
            }
            ...
            $lessonPlan->institution_id = $institutionId; // Keep institution from logged-in teacher
            $lessonPlan->teacher_id = $teacherId; // Keep teacher as logged-in teacher
