/**
 * Teacher Lesson Plans Management JavaScript
 * Handles cascading dropdowns for Classes -> Subjects
...
function initTeacherLessonPlanForm() {
    // Global variables
...
function loadSubjectsByInstitutionClass(institutionId, classId) {
    ...
    $.ajax({
        url: `/teacher/lesson-plans/subjects/${institutionId}/${classId}`,
        type: 'GET',
...
function setupEditForm() {
    // Set class and trigger change to load subjects
    if (window.currentLessonPlan.classId) {
        $('#class_id').val(window.currentLessonPlan.classId);
        $('#class_id').trigger('change.lessonPlans');
    }
}
...
function submitLessonPlanForm(action) {
    ...
    if (action === 'create') {
        ...
        url = `/teacher/lesson-plans/${lessonPlanId}`;
        method = 'POST';
    }
...
function createLessonPlanRow(lessonPlan) {
    ...
    const fileButtons = lessonPlan.lesson_plan_file ? 
        ...
        <a href="/teacher/lesson-plans/${lessonPlan.id}/download" class="btn btn-sm btn-outline-success" title="Download PDF">
...
function initRowEventHandlers(row) {
    ...
    row.find('.lesson-plan-status-select').off('change.lessonPlans').on('change.lessonPlans', function(e) {
        ...
        $.ajax({
            url: `/teacher/lesson-plans/${lessonPlanId}/status`,
            type: 'POST',
            data: {
    ...
    row.find('.edit-lesson-plan').off('click.lessonPlans').on('click.lessonPlans', function() {
        ...
        $.ajax({
            url: `/teacher/lesson-plans/${lessonPlanId}/edit`,
            type: 'GET',
            success: function(response) {
                if (response.success) {
                    ...
                    // Set class and trigger change to load subjects
                    $('#class_id').val(lessonPlan.class_id);
                    $('#class_id').trigger('change.lessonPlans');
...
function resetForm() {
    ...
    // Reset dropdowns (teacher is auto-set, so only reset class and subject)
