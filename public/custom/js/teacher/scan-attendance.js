...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (scanClass) {
        scanClass.addEventListener('change', function() {
            ...
            if (classId) {
                fetch(`/teacher/attendance/sections/${classId}`)
                    .then(res => res.json())
                    .then(data => {
                        ...
                        document.getElementById('scan-section-field').style.display = 'block';
                    });
            }
        });
    }
    ...
    function handleScanResult(scanValue) {
        ...
        const formData = {
            ...
            class_id: scanClass.value,
            section_id: scanSection.value,
            ...
            if (data.success) {
                scanResults.innerHTML = `
                    <div class="alert alert-success">
                        ...
                        <p class="mb-0">Student: ${data.user.name}</p>
                    </div>
                `;
                // Clear manual input
                manualInput.value = '';
