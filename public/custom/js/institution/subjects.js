...
if (
    $(".subject-status-select").length > 0 ||
    window.location.pathname.includes("/institution/subjects")
) {
...
$("#add-subject").on("click", function (e) {
    ...
    // Make AJAX request
    $.ajax({
        url: "/institution/subjects/store",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
$(document).on("click", ".edit-subject", function (e) {
    ...
    $.ajax({
        url: `/institution/subjects/edit/${subjectId}`,
        type: "GET",
        success: function (response) {
            if (response.success) {
...
$(document).on("click", "#update-subject", function (e) {
    ...
    $.ajax({
        url: `/institution/subjects/update/${formData.get("id")}`,
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
$(document).on("click", ".delete-subject", function (e) {
    ...
    $("#deleteForm").attr(
        "action",
        `/institution/subjects/delete/${subjectId}`
    );
...
$("#deleteForm").on("submit", function (e) {
    ...
    if (!window.location.pathname.includes("/institution/subjects")) {
        return;
    }
