...
$(document).ready(function () {
    ...
    if ($(".class-status-select").length > 0 || window.location.pathname.includes('/institution/classes')) {
    ...
    $("#update-class").on("click", function (e) {
        ...
        $.ajax({
            url: `/institution/classes/update/${classId}`,
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            ...
            error: function (xhr) {
                let errorMessage = "An error occurred while updating the class";
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    errorMessage = xhr.responseJSON.message;
                }
                toastr.error(errorMessage);
    ...
    $(document).on("click", ".delete-class", function (e) {
        ...
        $("#deleteForm").attr("action", `/institution/classes/delete/${classId}`);
    ...
    $("#deleteForm").on("submit", function (e) {
        ...
        if (!window.location.pathname.includes('/institution/classes')) {
            return;
        }
            ...
            classes.forEach(cls => {
                ...
                if (cls.section_ids && cls.section_ids.length > 0) {
                    ...
                    if (cls.sections && cls.sections.length > 0) {
                        sectionNames = cls.sections.map(s => s.name).join(", ");
                    ...
                    <tr>
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="ms-2">
                                    <h6 class="fs-14 mb-0">${escapeHtml(cls.name)}</h6>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="ms-2">
                                    <h6 class="fs-14 mb-0">${escapeHtml(sectionNames)}</h6>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="ms-2">
                                    <h6 class="fs-14 mb-0">${cls.student_count || 0}</h6>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div>
                                <select class="form-select class-status-select" data-class-id="${cls.id}">
                        ...
                        <td>
                            <div class="d-inline-flex align-items-center">
                                <a href="javascript:void(0);" class="btn btn-icon btn-sm btn-outline-white border-0 edit-class" 
                                   data-class-id="${cls.id}" data-class-name="${escapeHtml(cls.name)}" 
    ...
    function addStatusChangeListeners() {
        ...
        $(".class-status-select").off("change");
        
        $(".class-status-select").on("change", function () {
            const classId = $(this).data("class-id");
    ...
    function updateClassStatus(classId, status) {
        $.ajax({
            url: `/institution/classes/${classId}/status`,
            type: "POST",
            data: {
                status: status,
