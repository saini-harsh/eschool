$(document).ready(function () {
    // Basic page detection and setup
    const isSubjectsPage =
        $(".datatable").length > 0 ||
        window.location.pathname.includes("/admin/subjects");
    if (isSubjectsPage) {
    ...
    // Always wire institution -> class cascade when the dropdowns exist
    if ($("#institution_id").length && $("#class_id").length) {
...
function updateSubjectsTable(subjects) {
    ...
    if (!subjects || subjects.length === 0) {
        ...
        subjects.forEach(function (subject) {
            ...
            const className = subject.school_class
...
function initializeFilterForm() {
    ...
    $(document).on("submit", "#filter-dropdown form", function (e) {
        ...
        $(this)
            .find('input[name="institution_ids[]"]:checked')
            .each(function () {
        ...
        const classIds = [];
        $(this)
            .find('input[name="class_ids[]"]:checked')
            .each(function () {
...
function initializeClearFilters() {
    // Clear all filters
    $('a[href="javascript:void(0);"]:contains("Clear All")').on(
        "click",
        function () {
            // Clear search
            $("#subject-search").val("");
    ...
    $('a[href="javascript:void(0);"]:contains("Reset")').on(
        "click",
        function () {
...
function loadClassesByInstitution(institutionId) {
    ...
    classSelect
        .prop("disabled", true)
        .html('<option value="">Loading classes...</option>');
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            classSelect
                .html('<option value="">Error loading classes</option>')
                .prop("disabled", false);
