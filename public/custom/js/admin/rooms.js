...
function handleCreateRoom(event) {
    ...
    fetch("/admin/exam-management/rooms/store", {
        method: "POST",
        body: formData,
        headers: {
        ...
        .then((data) => {
            if (data.success) {
                ...
                if (designLayoutNow && data.data && data.data.id) {
                    setTimeout(() => {
                        window.location.href = `/admin/exam-management/rooms/${data.data.id}/design-layout`;
                    }, 1000);
                } else {
...
function viewRoom(roomId) {
    ...
    window.open(`/admin/exam-management/rooms/${roomId}`, "_blank");
}

function designLayout(roomId) {
    // Navigate to layout design page
    window.location.href = `/admin/exam-management/rooms/${roomId}/design-layout`;
}

function editRoom(roomId) {
    // Open edit modal or navigate to edit page
    fetch(`/admin/exam-management/rooms/${roomId}/edit`)
...
function handleEditRoom(roomId, event) {
    ...
    fetch(`/admin/exam-management/rooms/${roomId}/update`, {
        method: "POST",
        body: formData,
        headers: {
            "X-CSRF-TOKEN": document
...
function deleteRoom(roomId) {
    if (
        ...
        fetch(`/admin/exam-management/rooms/${roomId}`, {
            method: "DELETE",
            headers: {
                "X-CSRF-TOKEN": document
                    .querySelector('meta[name="csrf-token"]')
